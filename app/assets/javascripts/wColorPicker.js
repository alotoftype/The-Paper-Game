	...
	ColorPicker.prototype =
	{
		generate: function ()
		{
			...
			$('<input type="text" class="_wColorPicker_customInput" value=""/>').keyup(function(e)
			{
				...
				if(hex.length === 7) {
					$this.customTarget.css('backgroundColor', hex);
				}
		...
		colorSelect: function($this, color)
		{
			...
			if($this.settings.onSelect) {
				$this.settings.onSelect.apply(this, [color]);
			}
		...
		appendToElement: function($element)
		{
			...
			if($this.settings.mode === 'flat') {
				$element.append($this.colorPicker);
			} else {
				//setup button
				...
				$this.paletteHolder.css({width: $this.width < 240 ? 240 : $this.width, height: $this.height}).hide();
				...
				{
					$(document).click(function(){if($this.paletteHolder.hasClass('active')) {
						$this.hidePalette($this);
					}});
		...
		showPalette: function(e, $this)
		{
			...
			if(offset.left - $(window).scrollLeft() + $this.width > $(window).width()) {
				left = -1 * ($this.width - $this.paletteHolder.parent().outerWidth(true));
			}
		...
		hidePalette: function($this)
		{
			...
			if($this.paletteHolder.hasClass('active'))
			{
				...
				if($this.settings.effect === 'slide')
				{
					$this.paletteHolder.stop(true, false).animate({height: 0, top: $this.slideTopToBottom}, $this.settings.hideSpeed, function(){$this.paletteHolder.hide();});
				}
				else if($this.settings.effect === 'fade')
				{
